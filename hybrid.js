...
async function main() {
  ...
  const frame = (now) => {
    ...
    let displayMode = DisplayMode[displayModeRadio.value]
    gl.uniform1i(u_displayMode, displayMode)
    if ("Density" === displayModeRadio.value) {
      displayModeRadio.value = previousMode
      worker.postMessage({ density: true })
    }
    previousMode = displayModeRadio.value
